...
extension IntShape {
    ...
    private struct Bridge {
        ...
        @inlinable
        var slice: Delaunay.Slice {
            Delaunay.Slice(a: a.vertex.index, b: b.vertex.index)
        }
    }
    ...
    func split(extraPoints: [IntPoint]?) -> MonotoneLayout {
        ...
        var slices = Array<Delaunay.Slice>()
            ...
            while i < n {
                ...
                case .extra:
                    ...
                    while j < dSubs.count {
                        ...
                        if Self.isTetragonContain(p: p, a: pA, b: pB, c: pC, d: pD) {
                            let hand = links[dSub.middle]
                            slices.append(Delaunay.Slice(a: hand.vertex.index, b: node.vertex.index))
    ...
    private static func isTriangleContain(p: IntPoint, a: IntPoint, b: IntPoint, c: IntPoint) -> Bool {
        let d1 = Self.sign(a: p, b: a, c: b)
        let d2 = Self.sign(a: p, b: b, c: c)
        let d3 = Self.sign(a: p, b: c, c: a)
    ...
    private static func isTetragonContain(p: IntPoint, a: IntPoint, b: IntPoint, c: IntPoint, d: IntPoint) -> Bool {
        return Self.isTriangleContain(p: p, a: a, b: b, c: c) || Self.isTriangleContain(p: p, a: a, b: c, c: d)
    }
...
private extension Array where Element == IntShape.Link {
