...
public extension IntShape {
    ...
    func delaunay(extraPoints: [IntPoint] = []) -> Delaunay {
        ...
        for index in layout.indices {
            IntShape.triangulate(index: index, links: &links, triangleStack: &triangleStack)
            triangleStack.reset()
        }
        
        var triangles = triangleStack.getTriangles()
        
        var sliceBuffer = Delaunay.SliceBuffer(vertexCount: links.count, slices: layout.slices)
