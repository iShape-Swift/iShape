...
public extension Delaunay {
    ...
    private struct Validator {
        ...
        init(maxArea: Float, space: Space) {
            self.space = space
            if maxArea > 0 {
        ...
        func testRegular(triangle: Triangle) -> Int {
            let a = space.float(triangle.vertices.a.point)
            let b = space.float(triangle.vertices.b.point)
            let c = space.float(triangle.vertices.c.point)

            let ab = a.sqrDistance(b)
            let ca = c.sqrDistance(a)
            let bc = b.sqrDistance(c)
    ...
    mutating func tessellate(maxArea: Float, space: Space) {
        let validator = Validator(maxArea: maxArea, space: space)
